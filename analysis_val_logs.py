...
def analyze_and_plot():
    ...
    plt.title("Deconstructing the WLPS Score Components", fontsize=14, pad=15)
    plt.tight_layout()
    ...
    df['WLPS_Delta'] = df['Fixed_WLPS'] - df['Flawed_WLPS']
    df_sorted = df.sort_values('WLPS_Delta').reset_index(drop=True)
    
    colors =['#d62728' if x < 0 else '#2ca02c' for x in df_sorted['WLPS_Delta']]
    
    plt.bar(df_sorted.index, df_sorted['WLPS_Delta'], color=colors, width=1.0)
    plt.axhline(0, color='black', linewidth=1.5)
    plt.title("Image-by-Image WLPS Change (Fixed - Flawed)", fontsize=14, pad=15)
    plt.xlabel("Validation Images (Ranked by Delta)")
    plt.ylabel("Change in Score (Positive = Fixed Model won)")
    
    wins = (df['WLPS_Delta'] > 0).sum()
    losses = (df['WLPS_Delta'] < 0).sum()
    
    # Adjusted text position to dynamically fit the chart
    y_max = df['WLPS_Delta'].max()
    y_min = df['WLPS_Delta'].min()
    ...
    print("\n--- QUICK DIAGNOSTICS ---")
    print(f"Flawed Avg WLPS: {df['Flawed_WLPS'].mean():.4f} | Fixed Avg WLPS: {df['Fixed_WLPS'].mean():.4f}")
